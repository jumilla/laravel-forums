<div class="row">
    @if($tags->count() > 0)
        <h3>{{ trans('forum.threads.thread-tag') }}</h3>
        ...
        <div class="_tag_description_container">
            <strong>{{ trans('forum.threads.thread-tag-message') }}</strong>
        ...
        <div style="display:none;" class="_tags">
            @foreach($tags as $tag)
                <div class="_tag" title="{{ $tag->name }}">
                    {{ Form::checkbox("tags[{$tag->id}]", $tag->id, isset($article) ? $article->hasTag($tag->id) : null, ['title' => $tag->name]) }}
